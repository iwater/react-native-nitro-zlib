...
-   📦 **Full Algorithm Support**: Supports Deflate, Inflate, Gzip, Gunzip, Brotli, and **LZO (miniLZO)**.
...
// LZO Compression (Sync)
const lzoCompressed = zlib.lzoCompressSync(input);

// LZO Decompression (Sync)
// - Path A: Automatic size detection (Convenience)
const lzoDecompressedA = zlib.lzoDecompressSync(lzoCompressed);
// - Path B: Explicit size (Highly optimized for MDict/known size cases)
...
| `lzoCompress` / `Sync` | ✅ Supported | Raw LZO1X-1 |
| `lzoDecompress` / `Sync` | ✅ Supported | Supports optional `outputLength` |
| `crc32` | ✅ Supported | |
| `constants` | ✅ Supported | |
...
1.  **Rust Core (`rust_c_zlib`)**: Wraps `flate2` and `brotli` crates for Zlib and Brotli.
2.  **C++ Core**: Integrated `miniLZO` for high-speed LZO compression.
3.  **C++ Bridge**: C++ interfaces automatically generated by Nitro Modules.
